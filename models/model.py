...
class Diff_RNTraj(nn.Module):
    ...
    def forward(self, spatial_A_trans, SE, src_index_seqs):
        """
        ...
        batchsize, max_src_len = src_index_seqs.shape
        
        id_embed = SE[src_index_seqs]  # B, T, d
        
        diff_noise, const_loss, x0_loss = diff_forward_x0_constraint(self.diff_model, id_embed, self.diffusion_param, SE, spatial_A_trans)
    ...
    def generate_data(self, spatial_A_trans, SE, batchsize, length, pre_dim):
        ...
        x0 = cal_x0_from_noise_ddpm(self.diff_model, self.diffusion_param, batchsize, length, pre_dim)  # B, T, 65
        x0_road = x0[:,:,:]  # B, T, 64
        B, T, F = x0_road.shape
