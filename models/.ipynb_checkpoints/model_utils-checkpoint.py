...
def touli(index_uli_dict, index_areacode_dict, rids, parameters, save_txt_num = 0):
    ...
    trg_len = rids.size(0)
    seqs = torch.zeros(trg_len, batch_size, 2).to("cuda:0")
    traj_save_path = './generate_data/{}_diffT{}_{}/'.format(parameters.dataset,parameters.diff_T,formatted_time)
    if not os.path.exists(traj_save_path):
        os.makedirs(traj_save_path)
    with open(traj_save_path + str(save_txt_num) +".txt", 'a+') as f:
        for bs in range(batch_size):
            ...
            for i in range(trg_len):
                rid = rids[i][bs]
                uli = index_uli_dict.get(str(rid),rid)
                areacode = index_areacode_dict.get(str(rid),rid)
                if rid >= parameters.id_size: continue #parameters.id_size:continue
                
                f.write("{}, {}, {}\n".format(uli, areacode, rid))
            #     print(pt.lat, pt.lng, rid, rate)
            # print("----------------")
    return seqs
