...
if __name__ == '__main__':
    ...
    # load dataset   
    with open(train_trajs_dir + 'index_seqs.bin', 'rb') as f:  #路段序列
        all_index_seqs = pickle.load(f)
        f.close()
    ...
    for epoch in tqdm(range(args.n_epochs)):
        ...
        new_log_vars, train_loss, train_const_loss, train_diff_loss, train_x0_loss = \
                train(model, spatial_A_trans, SE, all_index_seqs, optimizer, log_vars, args, diffusion_hyperparams)
        scheduler.step()
